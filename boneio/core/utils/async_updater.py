...
class AsyncUpdater:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        has_sync_update = (
            hasattr(self, "update") and callable(self.update) and self.__class__.update is not AsyncUpdater.update
        )
        ...
        _LOGGER.debug(f"{self.__class__.__name__} initialized with {update_type} update method")
        ...
        try:
            ...
            while True:
                ...
                if use_async:
                    ...
                    update_interval = (
                        await self.async_update(timestamp=timestamp) or self._update_interval.total_in_seconds
                    )
                else:
                    # Use sync update (fallback)
                    update_interval = self.update(timestamp=timestamp) or self._update_interval.total_in_seconds
                # Sleep until update_interval expires OR wakeup event is triggered
                try:
                    await asyncio.wait_for(self._wakeup_event.wait(), timeout=update_interval)
