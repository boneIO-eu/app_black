...
class ESPHomeRemoteDevice(RemoteDevice):
    ...
    def __init__(
        ...
        _LOGGER.info("Configured ESPHome remote device '%s' (host=%s:%d)", name, host, port)
    ...
    def register_binary_sensor_callback(self, sensor_id: str, callback: Any) -> None:
    ...
    def register_output_callback(self, entity_id: str, callback: Any) -> None:
    ...
    async def start_connection(self) -> None:
        ...
        _LOGGER.info(
            "Started persistent connection to ESPHome device '%s' at %s:%d", self._name, self._host, self._port
        )
    ...
    async def _on_connect(self) -> None:
        ...
        try:
            ...
            _LOGGER.debug(
                ...
                len(self._light_keys),
                len(self._cover_keys),
                self._name,
            )
            ...
            if isinstance(state, SwitchState):
                ...
                for light in self._lights:
                    if light.get("key") == state.key:
                        ...
                        brightness_255 = round(state.brightness * 255) if state.state else 0
                        ...
                        _LOGGER.debug(
                            "Light '%s' state: on=%s, brightness=%.2f", light_id, state.state, state.brightness
                        )
                ...
                for cover in self._covers_list:
                    if cover.get("key") == state.key:
                        ...
                        last_op = self._cover_states.get(cover_id, {}).get(
                            "last_known_operation", 2
                        )  # Default to CLOSING (2)
                        if current_op != 0:
                            last_op = current_op
    ...
    async def discover_entities(self) -> dict[str, Any]:
        ...
        try:
            ...
            _LOGGER.info("Discovering entities on '%s' (%s)", device_info.name, device_info.mac_address)
            ...
            for entity in entities:
                if isinstance(entity, SwitchInfo):
                    ...
                    _LOGGER.debug(
                        "Found light: %s (%s) - brightness=%s, color_temp=%s, rgb=%s",
                        entity.name,
                        entity.object_id,
    ...
    def _get_entity_key(
        self, entity_id: str, entity_list: list[dict[str, Any]], key_cache: dict[str, int]
    ) -> int | None:
    ...
    class ESPHomeServiceListener(ServiceListener):
        ...
        def add_service(self, zc: Zeroconf, type_: str, name: str) -> None:
            ...
            if info:
                ...
                _LOGGER.debug("Found ESPHome device: %s at %s:%d", device_name, ip_address, info.port or 6053)
...
async def scan_esphome_devices(timeout: float = 3.0) -> dict[str, Any]:
    ...
    if not ZEROCONF_AVAILABLE:
        return {"devices": [], "error": "zeroconf not installed - mDNS discovery disabled"}
