...
class MCPOutput(BasicOutput):
    ...
    def __init__(
        self, pin: int, mcp: MCP23017, mcp_id: str, output_type: str = SWITCH, restored_state: bool = False, **kwargs
    ) -> None:
    ...
    def init_with_check_if_can_restore_state(self, restored_state: bool) -> None:
        if (
            restored_state
            and self._interlock_manager
            and self._interlock_groups
            and not self._interlock_manager.can_turn_on(self, self._interlock_groups)
        ):
