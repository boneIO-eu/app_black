...
class GpioManager:
    ...
    def add_input(
        ...
        if self._running:
            _LOGGER.error(
                "Cannot add new GPIO pin %s while manager is running. This pin was not configured at startup.", pin
            )
            return
        ...
        _LOGGER.debug("Registered input %s on pin %s (chip%d, line%d)", name, pin, chip, line)
    ...
    async def start(self, debug_mode: bool = False) -> None:
        ...
        for chip, chip_definitions in grouped_inputs.items():
            ...
            if debug_mode:
                ...
                successful_lines, failed_lines = await self._start_debug_mode(chip, chip_definitions, config, consumer)
            ...
            summary = ", ".join(f"{alias} (line {line})" for (_, line), alias in alias_map.items())
...
def get_gpio_manager(loop: asyncio.AbstractEventLoop | None = None, debounce_ms: int = 7) -> GpioManager:
